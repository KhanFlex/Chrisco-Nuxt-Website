        ...
        >
          <div class="tribe-common-l-container tribe-events-l-container">
            <div class="tribe-events-calendar-latest-past">
              <!-- Has data -->
              <Event
                v-if="data?.stories && !pending"
                v-for="event in data?.stories"
                :event="event"
              ></Event>

              <!-- Waiting for data -->
              <LoadingData v-if="pending" label="Church Events"/>

              <!-- Data is empty or has an error -->
              <EmptyData v-if="!data?.stories?.length && !pending" label="Church Events" />
            </div>
          </div>
        </div>
      </div>
...
const { pending, data } = await useFetch(
  "https://api.storyblok.com/v2/cdn/stories?starts_with=all-events&token=jmrSXvK178kWedQ67oQLoAtt",
  {
    lazy: true,
    headers: {
